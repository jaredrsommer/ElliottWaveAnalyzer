"""
Generate Images for README Documentation

This script generates professional chart images showcasing the Enhanced
Elliott Wave Analyzer system using real XRP/USD data.

Images generated:
1. pattern_detection.png - Elliott Wave pattern with probability
2. wave_line_segments.png - Wave structure with connecting lines
3. fibonacci_targets.png - Multiple Fibonacci targets displayed
4. historical_labeling.png - Complete historical wave annotation
5. trend_channels.png - Trend channels and Fibonacci levels
...
def download_xrp_data():
    """Download XRP/USD data from Yahoo Finance."""
    print("📊 Downloading XRP/USD data from Yahoo Finance...")
...
def plot_pattern_detection(df, analyzer, result):
    ...
    if result['invalidation_price']:
        ax1.axhline(y=result['invalidation_price'], color='red', linestyle='--', linewidth=2,
                   alpha=0.7, label=f"Stop Loss: ${result['invalidation_price']:.4f}")
    ...
    # Add statistics box
    stats_text = f"""Pattern Probability: {result['probability']:.1f}%
Fibonacci Score: {result['fibonacci_score']:.1f}%
Confidence: {result['confidence']:.1f}%
Wave Type: {result['wave_type'].upper()}"""
    ...
    values = [result['probability'], result['fibonacci_score'], result['confidence']]
...
def plot_fibonacci_targets(df, analyzer, result):
    ...
    if len(waves) >= 4:
        ...
        fib_618_w2 = wave1.high - (wave1_range * 0.618)
        fib_50_w2 = wave1.high - (wave1_range * 0.50)

        ax.axhline(y=fib_618_w2, color='purple', linestyle=':', linewidth=1.5,
                  alpha=0.5, label='Wave 2: 61.8% Fib')
        ax.axhline(y=fib_50_w2, color='purple', linestyle=':', linewidth=1.5, alpha=0.5)

        # Wave 4 Fib levels
        wave3_range = wave3.high - wave3.low
        fib_382_w4 = wave3.high - (wave3_range * 0.382)
        fib_236_w4 = wave3.high - (wave3_range * 0.236)

        ax.axhline(y=fib_382_w4, color='orange', linestyle=':', linewidth=1.5,
                  alpha=0.5, label='Wave 4: 38.2% Fib')
        ax.axhline(y=fib_236_w4, color='orange', linestyle=':', linewidth=1.5, alpha=0.5)
    ...
    if result['risk_reward_ratio']:
        rr_text = f"Risk/Reward Ratio: {result['risk_reward_ratio']:.2f}:1"
...
def plot_historical_labeling(df):
    ...
    for i, pattern_info in enumerate(results['all_patterns'][:5]):  # Show first 5 patterns
    ...
    # Add statistics
    stats_text = f"""Patterns Found: {results['num_patterns']}
Labeled Waves: {results['num_labeled_waves']}
Scan Coverage: {len(df_subset)} candles"""
    ...
    print(f"✓ Saved: {output_path}")
    print(f"  Found {results['num_patterns']} patterns, labeled {results['num_labeled_waves']} waves")
    return output_path
...
def main():
    ...
    # Download data
    df = download_xrp_data()

    # Save data for future use
    data_path = OUTPUT_DIR.parent / 'XRP_USD_data.csv'
    ...
    df_recent = df.iloc[-200:].copy().reset_index(drop=True)

    analyzer = EnhancedWaveAnalyzer(df_recent, min_probability=70.0)

    # Find best impulse pattern
    best_result = None
    for idx_start in range(0, len(df_recent) - 100, 10):
